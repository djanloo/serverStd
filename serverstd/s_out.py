...
body_string = body_string.format(startsession=datetime.now().strftime("%H:%M:%S"), content="{content}")
...
def sprint(string):
    ...
    current_msg += outblock_string.format(msg_counter=msg_counter, 
                                        content=string, 
...
def sperr(string):
    string = str(string)
    global current_msg, msg_counter
    current_msg += errblock_string.format(msg_counter=msg_counter, 
                                            content=string, 
                                            time=datetime.now().strftime("%H:%M:%S"))
    with open(FILE, "w") as f:
        f.write(body_string.format(content=current_msg))
...
def splot(figure):
    ...
    current_msg += img_string.format(img_counter=img_counter, file=f"/images/image_{img_counter}.png", time=datetime.now().strftime("%H:%M:%S"))
    with open(FILE, "w") as f:
        f.write(body_string.format(content=current_msg))
    img_counter+=1
